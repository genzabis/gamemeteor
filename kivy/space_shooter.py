...
class SpaceShooterGame(Screen):
    ...
    def end_game(self):
        ...
        label = Label(text="GAME OVER\nTekan Restart", font_size='40sp', bold=True,
                      pos=(self.width / 2 - 100, self.height / 2), halign="center")
        self.add_widget(label)
        
        # Menambahkan tombol restart
        restart_button = Button(text="Restart", size_hint=(None, None), size=(200, 60),
                                pos=(self.width / 2 - 100, self.height / 2 - 80))
        restart_button.bind(on_press=self.restart_game)  # Menghubungkan tombol dengan fungsi restart
        self.add_widget(restart_button)

    def restart_game(self, instance):
        # Menghapus elemen-elemen yang ada
        self.canvas.clear()
        self.bullets.clear()
        self.asteroids.clear()
        self._setup_game()  # Menyiapkan ulang permainan
        self.game_over = False
        self.score = 0  # Reset skor
        self.score_label.text = f"Skor: {self.score}"  # Memperbarui skor
        Clock.schedule_interval(self.update, 1 / 60.0)  # Memulai kembali pembaruan permainan
